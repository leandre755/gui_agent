...
from .paths import get_screenshots_dir, get_videos_dir
...
try:
    ...
    if FastMCP is None:
        raise AttributeError("FastMCP non trouvé dans mcp.server.fastmcp")
except (ImportError, AttributeError):
    try:
        import mcp.server.mcpserver as _mcpserver_mod

        FastMCP = getattr(_mcpserver_mod, "MCPServer", None)
        if FastMCP is None:
            raise AttributeError("MCPServer non trouvé dans mcp.server.mcpserver")
...
# Répertoires de stockage des artefacts et caches (résolution dynamique selon l'OS)
SCREENSHOTS_DIR = os.path.abspath(get_screenshots_dir())
os.makedirs(SCREENSHOTS_DIR, exist_ok=True)

VIDEOS_DIR = os.path.abspath(get_videos_dir())
...
def gui_window_resize_move(window_id: int, x: int, y: int, width: int, height: int) -> dict[str, Any]:
    ...
    if width <= 0 or height <= 0:
        return {"status": "error", "message": "width et height doivent être strictement positifs."}
...
def gui_find_template(template_path: str, threshold: float = 0.8, monitor_index: int = 1) -> dict[str, Any]:
    ...
    try:
        ...
        return {"status": "error", "message": "Seuil 'threshold' invalide (nombre attendu)."}
...
def gui_mouse_scroll(clicks: int, direction: str = "down") -> dict[str, Any]:
    ...
    try:
        ...
        for _ in range(clicks):
            if not run_xdotool(["click", btn]):
                return {"status": "error", "message": f"Échec du défilement vers le '{dir_clean}' via xdotool."}
            time.sleep(0.02)
...
def _validate_video_recording_params(
    ...
    if not output_path:
        timestamp = int(time.time())
        target_dir = VIDEOS_DIR if os.path.isdir(VIDEOS_DIR) else SCREENSHOTS_DIR
        output_path = os.path.join(target_dir, f"recording_{timestamp}.mp4")
...
def gui_start_video_recording(
    ...
    with _video_recording_lock:
        ...
        cmd = [
            ...
            "x11grab",
            "-i",
            f"{display_str if '.' in display_str.split(':')[-1] else f'{display_str}.0'}+{left},{top}",
            "-c:v",
            "libx264",
            "-preset",
            "ultrafast",
