...
def run_integration_test():
    ...
    if not isinstance(width, int) or not isinstance(height, int) or width <= 0 or height <= 0:
        print(f"Échec : Résolution invalide ({width}x{height}) !", file=sys.stderr)
        sys.exit(1)
    ...
    if screenshot_res.get("status") != "success":
        print("Échec : La capture a échoué !", file=sys.stderr)
        sys.exit(1)
    screenshot_path = screenshot_res.get("screenshot_path")
    if not screenshot_path or not os.path.exists(screenshot_path):
        print(f"Échec : Fichier image absent ({screenshot_path}) !", file=sys.stderr)
        sys.exit(1)
    ...
    for ox, oy in move_offsets:
        ...
        if res.get("status") != "success":
            print("Échec : Le mouvement a échoué !", file=sys.stderr)
            sys.exit(1)
        time.sleep(0.1)
    ...
    try:
        ...
        screenshots_dir = os.path.dirname(screenshot_path) if screenshot_path else "screenshots/"
