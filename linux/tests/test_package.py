...
def test_installation_and_uninstallation_scripts_presence():
    ...
    assert os.path.isfile("linux/install.sh"), "linux/install.sh manquant"
    assert os.path.isfile("windows/install.ps1"), "windows/install.ps1 manquant"
    assert os.path.isfile("linux/uninstall.sh"), "linux/uninstall.sh manquant"
    assert os.path.isfile("windows/uninstall.ps1"), "windows/uninstall.ps1 manquant"
    assert os.path.getsize("linux/install.sh") > 100
    assert os.path.getsize("windows/install.ps1") > 100
    assert os.path.getsize("linux/uninstall.sh") > 100
    assert os.path.getsize("windows/uninstall.ps1") > 100
...
def test_uninstall_script_cleans_standalone_atspi_mediator(tmp_path):
    ...
    res_dry = subprocess.run(
        ["bash", "linux/uninstall.sh", "--dry-run", "-y"],
        env=env,
        capture_output=True,
        text=True,
        check=False,
...
def test_modular_architecture_scaffolding(monkeypatch):
    ...
    def mock_run(c: list[str], **kw: object) -> object:
        emitted.append(c)
