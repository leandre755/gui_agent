...
def validate_video_recording_params(
    ...
    if not default_dir:
        try:
            ...
            default_base = get_videos_dir()
        except Exception:
            default_base = tempfile.gettempdir()
    else:
        default_base = default_dir

    path = output_path or os.path.join(default_base, f"recording_{uuid.uuid4().hex}.mp4")
